...
#include "game/client/ClientEffect.h"
#include "game/Projectile.h"
#include "game/ai/AI_Manager.h"
...
class LightningFence:rvWeapon {
public:
	idScriptObject *scriptObject;
	rvViewWeapon *viewModel;
	idAnimatedEntity *worldModel;
	CLASS_PROTOTYPE(LightningFence);
	idPlayer *owner;
	LightningFence(void);
	~LightningFence(void);
...
protected:
	...
	LightningPath						currentPath;

	// Chain lightning mod
	idList<LightningPath>				chainLightning;
...
private:
	...
	CLASS_STATES_PROTOTYPE(LightningFence);
};

CLASS_DECLARATION (idEntity,LightningFence)
EVENT(EV_Lightninggun_RestoreHum, LightningFence::Event_RestoreHum)
END_CLASS


/*
================
LightningFence::LightningFence
================
*/
LightningFence::LightningFence(void) {
...
/*
================
LightningFence::Spawn
================
*/
void LightningFence::Spawn(void) {
	gameLocal.Printf("fence?\n");
	weaponDef = gameLocal.FindEntityDef("lightning_fence");
	spawnArgs = weaponDef->dict;
	//gameLocal.FindEntity()
	idPlayer* _owner;
	_owner = owner;
...
================
*/
void LightningFence::Save(idSaveGame* savefile) const {
	int i;
...
/*
================
LightningFence::Restore
================
*/
void LightningFence::Restore(idRestoreGame* savefile) {
	int   i;
	int   num;
	float f;
...
/*
================
LightningFence::Think
================
*/
void LightningFence::Think(void) {
	trace_t	  tr;
...
// jshepard: allow projectile hits
	...
	this->worldModel->GetPosition(start, ax);//GetPosition(start, ax);
	idVec3 end;
...
/*
================
LightningFence::Attack
================
*/
void LightningFence::Attack(idEntity* ent, const idVec3& dir, float power) {
	// Double check
...
/*
================
LightningFence::UpdateChainLightning
================
*/
void LightningFence::UpdateChainLightning(void) {
	int					i;
	LightningPath*	parent;
	LightningPath*	path;
	idActor*			target;
...
/*
================
LightningFence::UpdateEffects
================
*/
void LightningFence::UpdateEffects(const idVec3& origin) {
	int					i;
	LightningPath*	parent;
	idVec3				dir;
...
/*
================
LightningFence::UpdateTrailEffect
================
*/
void LightningFence::UpdateTrailEffect(rvClientEffectPtr& effect, const idVec3& start, const idVec3& end, bool view) {
	idVec3 dir;
	dir = end - start;
	dir.Normalize();
...
/*
================
LightningFence::StopChainLightning
================
*/
void LightningFence::StopChainLightning(void) {
	int	i;
...
/*
================
LightningFence::ClientStale
================
*/
/*void LightningFence::ClientStale(void) {
...
/*
================
LightningFence::PreSave
================
*/
void LightningFence::PreSave(void) {
...
/*
================
LightningFence::PostSave
================
*/
void LightningFence::PostSave(void) {
	//restore the humming
...
/*
================
LightningFence::UpdateTubes
================
*/
void LightningFence::UpdateTubes(void) {
	idAnimator* animator;
...
CLASS_STATES_DECLARATION(LightningFence)
//STATE("Raise", LightningFence::State_Raise)
//STATE("Lower", LightningFence::State_Lower)
//STATE("Idle", LightningFence::State_Idle)
STATE("Fire", LightningFence::State_Fire)
END_CLASS_STATES

/*
================
LightningFence::State_Raise
================
*/
/*stateResult_t LightningFence::State_Raise(const stateParms_t& parms) {
	enum {
		STAGE_INIT,
		STAGE_WAIT,
	};
	...
	case STAGE_INIT:
		...
		weapon->PlayAnim(ANIMCHANNEL_ALL, "raise", parms.blendFrames);
		return SRESULT_STAGE(STAGE_WAIT);

		// Wait for the weapon to finish raising and allow it to be cancelled by
		// lowering the weapon 
	case STAGE_WAIT:
		if (weapon->AnimDone(ANIMCHANNEL_ALL, 4)) {
			weapon->SetState("Idle", 4);
			return SRESULT_DONE;
		}
		if (wsfl.lowerWeapon) {
...
/*
================
LightningFence::State_Lower
================
*/
/*
stateResult_t LightningFence::State_Lower(const stateParms_t& parms) {
	enum {
		STAGE_INIT,
		STAGE_WAIT,
		STAGE_WAITRAISE
...
/*
================
LightningFence::State_Idle
================
*/
/*
stateResult_t LightningFence::State_Idle(const stateParms_t& parms) {
	enum {
		STAGE_INIT,
		STAGE_WAIT,
	};
...
/*
================
LightningFence::State_Fire
================
*/
stateResult_t LightningFence::State_Fire(const stateParms_t& parms) {
	enum {
		STAGE_INIT,
		STAGE_ATTACKLOOP,
		STAGE_DONE,
	...
	case STAGE_DONEWAIT:
		...
		if (true){//!wsfl.lowerWeapon && wsfl.attack && AmmoAvailable()) {
			SetState("Fire", 4);
			return SRESULT_DONE;
		}
		return SRESULT_WAIT;
...
/*
================
LightningFence::Event_RestoreHum
================
*/
void LightningFence::Event_RestoreHum(void) {
	StopSound(SND_CHANNEL_BODY3, false);
	StartSound("snd_idle_hum", SND_CHANNEL_BODY3, 0, false, NULL);
}
...
/*
================
LightningPath::StopEffects
================
*/
void LightningPath::StopEffects(void) {
	if (trailEffect) {
		trailEffect->Stop();
...
/*
================
LightningPath::UpdateEffects
================
*/
void LightningPath::UpdateEffects(const idVec3& from, const idDict& dict) {
	idVec3 dir;
	dir = origin - from;
	dir.Normalize();
...
================
*/
void LightningPath::Save(idSaveGame* savefile) const {
...
/*
================
LightningPath::Restore
================
*/
void LightningPath::Restore(idRestoreGame* savefile) {
